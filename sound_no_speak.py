...
# Hugging Face token
HF_TOKEN = os.environ.get("HF_TOKEN", "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")
...
def generate_song(request: SongRequest):
    """
    Generate a song from a text prompt with no lyrics, then auto-play it on the PC.
    """
    if not request.prompt or not request.prompt.strip():
        raise HTTPException(status_code=400, detail="The 'prompt' field cannot be empty.")
    if not request.instrumental and (not request.lyrics or not request.lyrics.strip()):
        raise HTTPException(status_code=400, detail="The 'lyrics' field cannot be empty when 'instrumental' is False.")
...
if __name__ == "__main__":
    uvicorn.run("sound_no_speak:app", host="0.0.0.0", port=8001, reload=True)
